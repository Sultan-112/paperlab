...
type Quote = {price:number; age:number; source:string; timestamp:number};
...
type State = {mode:string; watches:string[]; quotes:Record<string,Quote>; decisions:Record<string,{action:string;reason:string}>; wallets:Wallet[]; positions:Position[]; trades:Trade[]; kill_switch:boolean; autopaper:boolean; catalog_counts:Record<string,number>; providers:Record<string,unknown>; replay:{paused:boolean; speed:number;cursor:number;total:number;finished:boolean}};
...
function App(){
 ...
 useEffect(()=>{
  ...
  const timer=setTimeout(()=>fetch(`/api/assets?q=${encodeURIComponent(query)}&market=${market}`,{headers:{Authorization:'Bearer '+token},signal:controller.signal})
 ...
 },[query,market,token,state?.catalog_counts.US,state?.catalog_counts.KSA,state?.catalog_counts.CRYPTO]);
 ...
 return <div className="shell">
  ...
  <section className="intro"><div><span className="eyebrow">YOUR MULTI-MARKET WORKSPACE</span><h2>Observe. Experiment. Understand.</h2><p>US equities, Saudi assets and crypto. Virtual capital. Local intelligence.</p></div><div className="mode">{state?.mode==='live'?'LIVE DATA':'REPLAY DATA'}<small>All execution stays simulated</small></div></section>
  ...
  <main><section className="panel universe"><div className="section-title"><h3>Asset universe</h3><span>DISCOVER</span></div><input aria-label="Search assets" placeholder="Search symbol or company…" value={query} onChange={e=>setQuery(e.target.value)}/><div className="tabs">{['','US','KSA','CRYPTO'].map(m=><button key={m} className={market===m?'active':''} onClick={()=>setMarket(m)}>{m||'All'}</button>)}</div><div className="asset-list">{assets.map(a=><button className="asset" key={a.id} onClick={()=>{setSelected(a.id);act('/watchlist/'+a.id,'PUT');}}><div><b>{a.id}</b><small>{a.name}</small></div><span>＋</span></button>)}{!assets.length&&<p>No matching assets. Refresh discovery or import a catalog.</p>}</div><button onClick={async()=>{const r=await act('/catalog/'+(market||'CRYPTO')+'/refresh');if(r)setError(JSON.stringify(r));}}>Refresh {market||'CRYPTO'} catalog</button><small>Search shows the first 100 matches. Refine your search. Catalog coverage depends on the source.</small></section>
  <section className="panel watch"><div className="section-title"><h3>Market watch</h3><span>1 SECOND ENGINE</span></div><div className="table-scroll"><table><thead><tr><th>Asset</th><th>Last price</th><th>Signal</th><th>Data source / age</th><th/></tr></thead><tbody>{state?.watches.map(a=><tr key={a} className={selected===a?'selected':''} onClick={()=>setSelected(a)}><td><b>{a}</b></td><td>{state.quotes[a]?number(state.quotes[a].price):'—'}</td><td><span className={'signal '+state.decisions[a]?.action}>{state.decisions[a]?.action||'HOLD'}</span></td><td>{state.quotes[a]?`${state.quotes[a].source} · ${Math.floor(state.quotes[a].age)}s`:'Waiting for source'}</td><td><button aria-label={'Remove '+a} onClick={e=>{e.stopPropagation();act('/watchlist/'+a,'DELETE');}}>×</button></td></tr>)}</tbody></table></div><p className="footnote">A one-second engine does not mean a new quote every second. Orders reject quotes older than 15 seconds.</p></section>
  <section className="panel ticket"><div className="section-title"><h3>Paper order</h3><span>VIRTUAL CAPITAL</span></div><label>Asset<select value={selected} onChange={e=>setSelected(e.target.value)}>{!state?.watches.includes(selected)&&<option>{selected}</option>}{state?.watches.map(a=><option key={a}>{a}</option>)}</select></label><label>Quantity<input type="number" min="0.00000001" step="any" value={quantity} onChange={e=>setQuantity(e.target.value)}/></label><div className="order-buttons"><button disabled={orderBusy||!connected||state?.kill_switch} onClick={()=>place('BUY')}>Paper buy</button><button disabled={orderBusy||!connected||state?.kill_switch} onClick={()=>place('SELL')}>Paper sell</button></div><small>10% order limit · 25% position limit<br/>0.10% fee · 0.05% adverse slippage<br/>No leverage or short positions.</small><hr/><button onClick={()=>act('/autopaper','PUT',{enabled:!state?.autopaper})}>{state?.autopaper?'Disable automatic paper trading':'Enable automatic paper trading'}</button><p className="footnote">100 quote-currency units per signal, at most once per asset per minute. The kill switch overrides automation.</p><hr/><h3>Why this signal?</h3><p>{decision?.reason||'Waiting for observations.'}</p><button disabled={busy} onClick={async()=>{setBusy(true);const r=await act('/explain/'+selected);if(r)setExplanation(`${r.source}: ${r.text}`);setBusy(false);}}>{busy?'Ollama is thinking…':'Explain with local AI'}</button><p className="explanation">{explanation}</p></section>
  ...
  </main><details className="panel"><summary>Provider diagnostics & catalog coverage</summary><pre>{JSON.stringify(state?.providers,null,2)}</pre><p>US: provider-wide discovery, at most 30 live IEX subscriptions. KSA: catalog completeness unverified; replay required for prices. Crypto: all active Spot pairs exposed by Binance.</p></details><footer>PaperLab / Reference implementation <span>Local infrastructure. Simulated execution. Educational signals.</span></footer>
 </div>;
}
