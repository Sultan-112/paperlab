...
type State = {asset_details:Record<string,{name:string;currency:string}>; mode:string; watches:string[]; quotes:Record<string,Quote>; decisions:Record<string,{action:string;reason:string}>; wallets:Wallet[]; positions:Position[]; trades:Trade[]; kill_switch:boolean; autopaper:boolean; catalog_counts:Record<string,number>; providers:Record<string,unknown>; replay:{paused:boolean; speed:number;cursor:number;total:number;finished:boolean}};
...
function App(){
 ...
 useEffect(()=>{
  ...
  const timer=setTimeout(()=>fetch(`/api/assets?q=${encodeURIComponent(query)}&market=${market}&offset=${page*100}`,{headers:{Authorization:'Bearer '+token},signal:controller.signal})
